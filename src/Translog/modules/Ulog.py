...
def convert_ulog2csv(ulog_file_name, messages, output, delimiter, full, id, disable_str_exceptions=False):
    """
    Coverts and ULog file to a CSV file.

    :param id: save num
    :param full: save type
    :param disable_str_exceptions:
    ...
    if (full):
        header = []
        with open(os.path.join(output_file_prefix + "_FULL" + ' (' + str(id) + ')' + ".csv"), 'w') as csvfile:
            w = csv.writer(csvfile, delimiter=delimiter, lineterminator="\n")
            for d in data:
                names = [f.field_name for f in d.field_data]
                names.remove('timestamp')
                names.insert(0, 'timestamp')
                for head in names:
                    header.append(d.name + '.' + head)
            w.writerow(header)
            i = 0
            b = len(d.data['timestamp'])
            while i < b:
                dataex = []
                for d in data:
                    ...
                    for k in range(len(data_keys)):
                        if len(d.data[data_keys[k]]) > i:
                            dataex.append(str(d.data[data_keys[k]][i]))
                        elif len(d.data[data_keys[k]]) > 0:
                            dataex.append("")
                i += 1
                w.writerow(dataex)
            csvfile.close()
        ...
        for d in data:
            ...
            # Сбор нужных параметров с таблиц
            if d.name == 'vehicle_global_position':
                if d.multi_id == 0:
                    data_keys = [f.field_name for f in d.field_data]
                    data_keys.remove('timestamp')
                    data_keys.insert(0, 'timestamp')  # we want timestamp at first position
                    timestamp = d.data[data_keys[0]]
                    latitude = d.data[data_keys[2]]
                    lontitude = d.data[data_keys[3]]

                    for i in range(len(timestamp)):
                        t = []
                        flatitude = ""
                        flontitude = ""
                        flatitude = str(latitude[i]).replace('.', '')
                        flontitude = str(lontitude[i]).replace('.', '')
            ...
            if d.name == 'vehicle_rates_setpoint':
                if d.multi_id == 0:
                    ...
                    data_keys.insert(0, 'timestamp')  # we want timestamp at first position
                    timestamp = d.data[data_keys[0]]
                    roll = d.data[data_keys[1]]
                    pitch = d.data[data_keys[2]]
                    yaw = d.data[data_keys[3]]
            ...
            if d.name == 'sensor_gps':
                if d.multi_id == 0:
                    ...
                    timestamp = d.data[data_keys[0]]
                    hspeed = d.data[data_keys[15]]
                    xspeed = d.data[data_keys[16]]
                    yspeed = d.data[data_keys[17]]
                    zspeed = d.data[data_keys[18]]
                    alt = d.data[data_keys[5]]

                    noise = d.data[data_keys[13]]
                    jamming = d.data[data_keys[14]]

                    gpsnum = d.data[data_keys[27]]
                    gpslevel = d.data[data_keys[24]]
            ...
            if d.name == 'battery_status':
                if d.multi_id == 0:
                    ...
                    timestamp = d.data[data_keys[0]]
                    voltage = d.data[data_keys[2]]
                    amp = d.data[data_keys[4]]
                    cell_count = d.data[data_keys[10]]

                    for i in range(len(timestamp)):
                        t = []
                        t.append(timestamp[i])
                        t.append(round(voltage[i] * amp[i] * cell_count[i], 2))
        ...
        # Заголовки оптимальной таблицы
        optional_finally.append(
            ['datetime', "flytime", "hSpeed", "distance", "longitude", "latitude", "height", "xSpeed", "ySpeed",
             "zSpeed",
             "pitch", "roll", "yaw", "FixTypeGps", "gpsNum", "noise", "jamming", "consumption"])
        optional_sorted = sorted(optional, key=lambda x: x[0])
        ...
        # Сортирока и объединение таблиц в одну оптимальную
        for i in optional_sorted:
            if len(i) == 10:
                hspeed = i[1]
                xspeed = i[2]
                yspeed = i[3]
            ...
            if len(i) == 2:
                ...
                if optional_finally[-1][0] == '' or optional_finally[-1][2] == "":
                    optional_finally[-1][0] = i[0]
                    optional_finally[-1][4] = i[1]
                    optional_finally[-1][5] = i[2]
                else:
                    optional_finally.append(
                        [
                            i[0],
        ...
        # Запись таблицы
        if len(optional_finally) > 0:
            with open(os.path.join(output_file_prefix + "_OPT" + ' (' + str(id) + ')' + ".csv"), 'w') as csvfile:
                w = csv.writer(csvfile, delimiter=delimiter, lineterminator="\n")
                for row in optional_finally:
                    if row[0] != '' and row[2] != '':
                        w.writerow(row)
                csvfile.close()
