...
public class GoTxt_12 extends Record {
    ...
    public void setStateStrings() {
        ...
        NGPE = ngc.name();
        GoTxt_12.FLYC_STATE fs = GoTxt_12.FLYC_STATE.find(flyc_state);
        FLCS = fs.name();
        GoTxt_12.FLIGHT_ACTION fa = GoTxt_12.FLIGHT_ACTION.find(flight_action);
        FLACTION = fa.name();
        GoTxt_12.FLYC_COMMAND fc = GoTxt_12.FLYC_COMMAND.find(flycCommand);
        FLYCCOMMAND = fc.name();
        ModeSwitch = "P";
        if (rcModeChannel == 1) {
            ModeSwitch = "Sport";
