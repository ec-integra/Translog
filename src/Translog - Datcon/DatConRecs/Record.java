...
public abstract class Record extends RecSpec {
    ...
    private boolean optcheck(String signal, String suffix) {
        if(TypeGo.equals("full")) {
            return true;
        }

        String[] opt_signal_valid = {"IMU_ATTI(0)", "GPS(0)", "General", "Controller"};
        String[][] opt_suffix_valid = {
                {
                        "velComposite", // hspeed
                        "Longitude", // longitude
                        "Latitude", // latitude
        ...
        int inum;
        for (String si : opt_signal_valid) {
            if (signal.equals(si)) {
                inum = Arrays.asList(opt_signal_valid).indexOf(signal);
                for (String su : opt_suffix_valid[inum]) {
                    if (suffix.equals(su)) {
                        return true;
                    }
                }
    ...
    private boolean coordtostr(String suffix) {
        String[] opt_suffixes = {"Longitude", "Latitude", "Long", "Lat"};
        for (String su : opt_suffixes) {
            if (suffix.equals(su)) {
                return true;
            }
        }
    ...
    public void printCsvValue(Number value, Signal signal, String suffix,
        ...
        if(optcheck(signal.getName(), suffix)) {
            String svalue = "";
            if(coordtostr(suffix)) {
                double newval = value.doubleValue();
                svalue = Double.toString(newval);
